...
func runOpcode(instr []int64, inputs ...int64) (ret *Intcode, err error) {
	ret = NewIntcode(instr...)
	ret.Inputs = append([]int64(nil), inputs...)
	err = ret.Step()
	return
}
...
func TestOpcode_Add(t *testing.T) {
	tests := []struct {
		...
		{[]int64{1, 0, 3, 5, 99, 0}, 6},
		{[]int64{101, 0, 3, 5, 99, 0}, 5},
		{[]int64{1001, 0, 3, 5, 99, 0}, 1004},
		{[]int64{1101, 0, 3, 5, 99, 0}, 3},
		{[]int64{1101, -1, -2, 5, 99, 0}, -3},
		{[]int64{11101, 0, 3, 5, 99, 0}, 3},
		{[]int64{10101, 0, 3, 5, 99, 0}, 5},
	}
...
func TestOpcode_Mult(t *testing.T) {
	tests := []struct {
		...
		{[]int64{2, 0, 3, 5, 99, -1}, 2 * 5},
		{[]int64{102, 3, 3, 5, 99, -1}, 3 * 5},
		{[]int64{1002, 0, 3, 5, 99, -1}, 1002 * 3},
		{[]int64{1102, 3, 3, 5, 99, -1}, 3 * 3},
		{[]int64{1102, 1, -1, 5, 99, -1}, 1 * -1},
		{[]int64{11102, 2, 3, 5, 99, -1}, 2 * 3},
		{[]int64{10102, 2, 3, 5, 99, -1}, 2 * 5},
	}
	for i, tt := range tests {
...
func TestOpcode_Input(t *testing.T) {
	ic, err := runOpcode([]int64{3, 3, 99, 0}, 1)
	assert.Nil(t, err)
	assert.Equal(t, 1, ic.CurrentInput)
	assert.Empty(t, ic.Outputs)
	assert.Equal(t, int64(2), ic.Pc)
}

func TestOpcode_Output(t *testing.T) {
	ic, err := runOpcode([]int64{4, 3, 99, 1})
	assert.Nil(t, err)
	assert.Equal(t, 0, ic.CurrentInput)
	assert.Equal(t, []int64{1}, ic.Outputs)
	assert.Equal(t, int64(2), ic.Pc)
	assert.Equal(t, ic.originalProgram, ic.Program)

	ic, err = runOpcode([]int64{104, 3, 99, 1})
	assert.Nil(t, err)
	assert.Equal(t, 0, ic.CurrentInput)
	assert.Equal(t, []int64{3}, ic.Outputs)
	assert.Equal(t, int64(2), ic.Pc)
	assert.Equal(t, ic.originalProgram, ic.Program)
}
...
func TestOpcode_JumpIfTrue(t *testing.T) {
	tests := []struct {
		...
		{[]int64{5, 0, 3, 1}, 1},
		{[]int64{105, 2, 0, 0}, 105},
		{[]int64{1105, 3, 10, 0}, 10},
		{[]int64{1005, 0, 2, 1}, 2},
		// False
		{[]int64{5, 3, 0, 0}, 3},
		{[]int64{105, 0, 0, 0}, 3},
		{[]int64{1105, 0, 1, 0}, 3},
		{[]int64{1005, 3, 4, 0, 10}, 3},
	}
	for i, tt := range tests {
...
func TestOpcode_LessThan(t *testing.T) {
	tests := []struct {
		...
		{[]int64{7, 0, 4, 4, 8}, 1},
		{[]int64{107, 4, 4, 4, 6}, 1},
		{[]int64{1007, 4, 4, 4, 2}, 1},
		{[]int64{1107, 10, 11, 4, 2}, 1},
		// False
		{[]int64{7, 4, 2, 4, 6}, 0},
		{[]int64{107, 109, 0, 4, 6}, 0},
		{[]int64{1007, 4, 10, 4, 11}, 0},
		{[]int64{1107, 11, 10, 4, 2}, 0},
	}
	for i, tt := range tests {
...
func TestOpcode_Equals(t *testing.T) {
	tests := []struct {
		...
		{[]int64{8, 0, 4, 4, 8}, 1},
		{[]int64{108, 4, 3, 4, 6}, 1},
		{[]int64{1008, 4, 10, 4, 10}, 1},
		{[]int64{1108, 10, 10, 4, 2}, 1},
		// False
		{[]int64{8, 4, 3, 4, 6}, 0},
		{[]int64{108, 109, 0, 4, 6}, 0},
		{[]int64{1008, 4, 10, 4, 9}, 0},
		{[]int64{1108, 10, 11, 4, 2}, 0},
	}
	for i, tt := range tests {
		t.Run(fmt.Sprintf("lessthan_%d", i), func(t *testing.T) {
			ic, err := runOpcode(tt.prog)
...
func Test_Day5Example1(t *testing.T) {
	for i, test := range []int64{-1, 0, 1, 5, 1000, 99} {
		ic := NewIntcode(3, 0, 4, 0, 99)
		t.Run(fmt.Sprintf("d5_e1__%d", i), func(t *testing.T) {
			ic.Reset()
			ic.Inputs = []int64{test}
			ic.MustRun()
			assert.Equal(t, []int64{test}, ic.Outputs)
		})
	}
}
...
func Test_Day5Examples(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			[]int64{-1, 0, 1, 8, 9, 99, 1000},
			[]int64{999, 999, 999, 1000, 1001, 1001, 1001},
		},
	}
	for i, test := range tests {
...
func Test_ChannelInputOutput(t *testing.T) {
	...
	o, ok := <-outputChan
	assert.True(t, ok)
	assert.Equal(t, int64(3), o)
	o, ok = <-outputChan
	assert.True(t, ok)
	assert.Equal(t, int64(2), o)
